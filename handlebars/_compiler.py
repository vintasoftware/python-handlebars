...
class strlist(list):
    ...
    def grow(self, thing):
        ...
        # This will only ever match in Python 2 since str is str in
        # Python 3.
        elif type(thing) == str:
...
def resolve_subexpr(helpers, name, context, *args, **kwargs):
    if name not in helpers:
        raise HandlebarsError(u"Could not find property %s" % (name,))
...
class FunctionContainer:
    ...
    def full_code(self):
        headers = (
            u'import handlebars\n'
            u'\n'
            u'if handlebars.__version__ != %s:\n'
            u'    raise handlebars.HandlebarsError("This template was precompiled with handlebars version %s, running version %%s" %% handlebars.__version__)\n'
            u'\n'
            u'from handlebars import strlist, Scope, HandlebarsError\n'
            u'from handlebars._compiler import _handlebars_, escape, resolve, resolve_subexpr, prepare, ensure_scope\n'
            u'\n'
            u'from functools import partial\n'
            u'\n'
            u'\n'
        ) % (repr(handlebars.__version__), handlebars.__version__)
...
class CodeBuilder:
    ...
    def start(self):
        ...
        if len(self.stack) == 1:
            self._result.grow([
                ...
                u"    _helpers = dict(_handlebars_['helpers'])\n"
    ...
    def add_partial(self, symbol, arguments):
            ...
            for argument in arguments:
                ...
                if kwmatch:
                    ...
                    if positional_args != 0:
                        raise HandlebarsError("An extra positional argument was passed to a partial")
                    positional_args += 1
                    arg = argument
        ...
        self._result.grow([
            ...
            u"        raise HandlebarsError('The partial %s could not be found' % partialName)\n",
...
class Compiler:
    ...
    def _generate_code(self, source):
        ...
        if not isinstance(source, str):
            raise HandlebarsError("Template source must be a unicode string")
        ...
        if position < len(source):
            ...
            raise HandlebarsError("Error at character %s of line %s near %s" % (char_num, line_num, word))
    ...
    def compile(self, source, path=None):
        ...
        def make_module_name(name, suffix=None):
            output = 'handlebars._templates.%s' % name
            if suffix:
                output += '_%s' % suffix
            return output
        ...
        filename = '%s.py' % mod_name.replace('handlebars.', '').replace('.', '/')
